...
void loop()
{
  ...
  // Positive means robot is to the left. (Will have to turn right)
  // Negative means robot is to the right. (Will have to turn left)
  // Also, if -2000 could mean turn left OR that there is no line at all.
  ...
  // improve performance.
  int power_difference = proportional/5 + integral/5000 + derivative*3/2;
  ...
  if(proportional > -500 && proportional < 500){
    //This works well for going in a straight line, but turns are too wide.
    if(power_difference < 0) //Turn Left
      set_motors(maximum + power_difference, maximum);
    else //Turn Right
      set_motors(maximum, maximum - power_difference);
  }
  else{
    if(proportional <= -500){ //Sharp Left Turn

      //Go past end of line a little bit
      while(proportional != -2000 && proportional != 2000){
          proportional = robot.readLine(sensors, IR_EMITTERS_ON) - 2000;
          set_motors(30,30);
      }
      
      while( proportional <= -250 || proportional >= 250 ){
        proportional = robot.readLine(sensors, IR_EMITTERS_ON) - 2000;
        set_motors(-maximum, maximum);
      }
